...
if(isset($_POST['recup_submit'],$_POST['recup_mail'])) {
   if(!empty($_POST['recup_mail'])) {
      ...
      if(filter_var($recup_mail,FILTER_VALIDATE_EMAIL)) {
         $mailexist = $bdd->prepare('SELECT ID_compte,nom FROM compte WHERE email = ?');
         ...
         $header.='From:"[VOUS]"<izidaniel@hotmail.fr>'."\n";
         ...
         ';
         mail($recup_mail, "Récupération de mot de passe - Maresa", $message, $header);
       header("Location:http://127.0.0.1/Projet_maresa/dossier_php/recupform.php?section=code");
         } else {
...
if(isset($_POST['verif_submit'],$_POST['verif_code'])) {
   if(!empty($_POST['verif_code'])) {
      ...
      if($verif_req == 1) {
         ...
         var_dump($up_req->execute(array($_SESSION['recup_mail'])));
         header('Location:http://127.0.0.1/Projet_maresa/dossier_php/recupform.php?section=changemdp');
      } else {
         $error = "Code invalide";
      }
   } else {
      ...
      if($verif_confirme == 1) {
         ...
         if(!empty($mdp) AND !empty($mdpc)) {
            if($mdp == $mdpc) {
               ...
               $ins_mdp = $bdd->prepare('UPDATE compte SET motdepasse = ? WHERE email = ?');
               $ins_mdp->execute(array($mdp,$_SESSION['recup_mail']));
               $del_req = $bdd->prepare('DELETE FROM recuperation WHERE mail = ?');
               $del_req->execute(array($_SESSION['recup_mail']));

               header('Location:http://127.0.0.1/Projet_maresa/dossier_php/connexion.php');
            } else {
